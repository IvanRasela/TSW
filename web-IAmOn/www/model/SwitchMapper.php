...
class PostMapper {
	...
	* @return mixed Array of Switch instances (without comments)
	*/
	...
	*
	* @throws PDOException if a database error occurs
	* @return Post The Switch instances (without comments). NULL
	...
	*/
	public function findByIdWithComments($postid){
		...
		*/
		public function save(Switch $switch) {
			$stmt = $this->db->prepare("INSERT INTO Switch(SwitchName, Private_UUID, Public_UUID, LastTimePowerOn, MaxTimePowerOn, DescriptionSwitch, SwitchState, AliasUser) values (?,?,?,?,?,?,?,?)");
			$stmt->execute(array($switch->getSwitchName(), $switch->getPrivate_UUID(), $switch->getPublic_UUID(),$switch->getLastTimePowerOn(),$switch->getMaxTimePowerOn(),$switch->getDescriptionSwicth(),$switch->getSwitchState(),$switch->getAliasUser()->getAlias()));
		...
		*
		* @param Switch $switch The switch to be deleted
		* @throws PDOException if a database error occurs
		* @return void
		*/
		public function delete(Switch $switch) {
			$stmt = $this->db->prepare("DELETE from Switch WHERE SwitchName=?");
			$stmt->execute(array($switch->getSwitchName()));
		}
